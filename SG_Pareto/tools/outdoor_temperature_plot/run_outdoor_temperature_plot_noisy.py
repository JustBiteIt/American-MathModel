...
NOISE_STD_C = 0.30
DIFF_THRESHOLD_C = 0.20
TARGET_DIFF_RATIO = 0.70
MIN_CORR = 0.75
...
def _smooth_noise(base: np.ndarray, window: int = 21) -> np.ndarray:
    s = pd.Series(base).rolling(window=window, center=True, min_periods=1).mean()
...
def _build_noise_doy(n: int, *, seed: int, use_ar1: bool) -> np.ndarray:
    rng = np.random.default_rng(seed)
    noise_raw = rng.normal(0.0, 1.0, size=n)
    noise = _smooth_noise(noise_raw, window=21)
    if use_ar1:
        noise = noise + _smooth_noise(_ar1_noise(n, seed=seed + 1), window=9)
...
<body>
  ...
  <script>
    ...
    for (let i = 0; i <= yTicks; i++) {
      ...
      ctx.fillStyle = "#555";
      ctx.font = "12px Arial";
      ctx.fillText(val.toFixed(1), 32, y + 4);
    }
    ...
    ctx.save();
    ctx.translate(10, pad.top + plotH / 2 + 40);
...
def build_plot(
    ...
    *,
    ...
    # build 2040 daily series with smooth noise so trend stays similar
    noise_base = _build_noise_doy(365, seed=int(ar1_seed), use_ar1=use_ar1_noise)
    abs_noise = np.abs(noise_base)
    q = float(np.quantile(abs_noise, 1.0 - TARGET_DIFF_RATIO))
    if q <= 1e-8:
        scale = NOISE_STD_C
    else:
        scale = DIFF_THRESHOLD_C / q
    noise_doy = noise_base * scale
    diff_ratio = float(np.mean(np.abs(noise_doy) >= DIFF_THRESHOLD_C))
    if diff_ratio < TARGET_DIFF_RATIO and diff_ratio > 0:
        scale = scale * (TARGET_DIFF_RATIO / diff_ratio)
        noise_doy = noise_base * scale
        diff_ratio = float(np.mean(np.abs(noise_doy) >= DIFF_THRESHOLD_C))
        for _ in range(10):
            if diff_ratio >= TARGET_DIFF_RATIO:
                break
            scale *= 1.05
            noise_doy = noise_base * scale
            diff_ratio = float(np.mean(np.abs(noise_doy) >= DIFF_THRESHOLD_C))

    daily_2040 = daily.values + float(delta_trend_C) + noise_doy
    corr = np.corrcoef(daily.values, daily_2040)[0, 1]
    if not np.isfinite(corr):
        corr = 0.0

    print(f"noise diff_ratio={diff_ratio:.4f}, corr={corr:.3f}, scale={scale:.3f}")
    ...
    doy_2040 = daily.index.to_numpy(int)
    ...
    ax.plot(doy, daily.values, color="#4c78a8", label="Baseline (daily mean)", linewidth=1.4)
    ax.plot(doy_2040, daily_2040, color="#f28e2b", label="2040 scenario (daily mean)", linewidth=1.4)
    ax.set_xlabel("Month")
    ...
    series_baseline = [{"x": int(x), "y": float(y)} for x, y in zip(doy, daily.values) if np.isfinite(y)]
    series_scenario = [{"x": int(x), "y": float(y)} for x, y in zip(doy_2040, daily_2040) if np.isfinite(y)]
    payload = {
        "meta": {
            "title": "Outdoor Temperature: Full Year",
            "x_min": int(min(doy.min(), doy_2040.min())),
            "x_max": int(max(doy.max(), doy_2040.max())),
            "noise_std_C": float(NOISE_STD_C),
        },
    ...
    return {
        ...
        "json": json_path,
        "html": html_path,
        "scenario_mean": float(np.mean(daily_2040)),
        "scenario_max": float(np.max(daily_2040)),
    }
...
def main() -> None:
    ...
    if weather_csv is None or not weather_csv.exists():
        raise SystemExit("找不到天气文件，请先运行 main_sg_pareto.py 生成 weather 文件。")
    ...
    scenario_mean = float(result["scenario_mean"])
    scenario_max = float(result["scenario_max"])

    print("Outdoor temperature plot (noisy 2040) generated:")
