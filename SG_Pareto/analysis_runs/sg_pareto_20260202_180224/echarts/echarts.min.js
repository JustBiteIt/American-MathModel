  ...
  function init(dom){
    ...
    function render(){
      ...
      for(let i=0;i<=nt;i++){
        ...
        ctx.stroke();
        ctx.fillStyle = "#222";
        ctx.fillText(vy.toFixed(0), left - 42, ty + 4);
      }

      // axis labels
      if(option.xAxis && option.xAxis.name){
        ctx.fillText(option.xAxis.name, left + plotW/2 - 80, h - 10);
      }
      ...
      series.forEach((s, idx) => {
        ...
        const size = s.symbolSize || 4;
        const opacity = (s.itemStyle && s.itemStyle.opacity != null) ? s.itemStyle.opacity : 0.6;
        ctx.fillStyle = col;
