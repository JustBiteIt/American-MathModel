    ...
    *,
    ...
    for k in range(n):
        ...
        if Ti_free < Theat:
            ...
            Phi_h[k] = max(0.0, Phi_req)
        elif Ti_free > Tcool:
            Tset = Tcool
            Phi_req = (Ci / dt) * (Tset - Ti[k]) - base
            Phi_h[k] = min(0.0, Phi_req)
        else:
            Phi_h[k] = 0.0
...
def simulate_2r2c_no_hvac(
    ...
    *,
    ...
    """
    2R2C without HVAC (no heating/cooling). Returns Ti (n+1), Tm (n+1).
    """
    Phi_s = np.asarray(Phi_s_W, dtype=float)
    Ta = np.asarray(T_out_C, dtype=float)
    n = len(Ta)
