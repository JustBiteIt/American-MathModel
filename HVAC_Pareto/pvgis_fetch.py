...
def _extract_series(rows: list[dict[str, Any]]) -> pd.DataFrame:
    ...
    # Find time column key robustly (e.g., "time", "time(UTC)", "Time (UTC)")
...
def fetch_pvgis_tmy_to_csv(
    *,
    ...
    """
    Fetch PVGIS TMY and write standardized weather CSV:
      columns = datetime,DNI,DHI,GHI,T_out
    ...
    request = Request(url, headers={"User-Agent": "HVAC_Pareto/1.0"})
    last_err = None
